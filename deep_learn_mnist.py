# -*- coding: utf-8 -*-
# @Time    : 2019/6/10 上午9:39
# @Author  : baby松
# @FileName: deep_learn_mnist.py
# @Software: PyCharm
"""
tensorflow deep learn mnist
"""
...
weights = {
    ...
    "w2": tf.Variable(tf.random_normal(shape=[n_unit_hidden_1, n_unit_hidden_2], stddev=0.2)),
...
biases = {
    ...
    "b2": tf.Variable(tf.random_normal(shape=[n_unit_hidden_2], stddev=0.2)),
...
def multiplayer_perceotron(_X, _weights, _biases):
    ...
    layer1 = tf.sigmoid(tf.add(tf.matmul(_X, _weights["w1"]), _biases["b1"]))

    # 第二层 -> 第三层 hidden1 -> hidden2
    layer2 = tf.sigmoid(tf.add(tf.matmul(layer1, _weights["w2"]), _biases["b2"]))
    ...
    while True:
        ...
        if (epoch + 1) % display_step == 0:
            ...
            if train_acc > 0.9 and test_acc > 0.9:
                saver.save(sess, './/MNIST/MNIST_data')
                break
        epoch += 1
